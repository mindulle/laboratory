...
Array elements are object properties in the same way that `toString` is
...
The [`reduce()`](array/reduce) and [`reduceRight()`](array/reduceright)
...
:   Returns `true` if every element in the calling array satisfies the
    testing function.
