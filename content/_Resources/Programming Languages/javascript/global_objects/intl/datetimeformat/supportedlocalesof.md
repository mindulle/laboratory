...
The `Intl.DateTimeFormat.supportedLocalesOf()` static method returns an
...
are supported in date and time formatting without having to fall back to
...
in date and time formatting, `supportedLocalesOf` returns the Indonesian
and German language tags unchanged, even though `pinyin` collation is
neither relevant to date and time formatting nor used with Indonesian,
...
--------------
  ...
  [\#
  sec-intl.datetimeformat.supportedlocalesof]](https://tc39.es/ecma402/#sec-intl.datetimeformat.supportedlocalesof)
