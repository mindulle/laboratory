...
The `Intl.Segmenter.supportedLocalesOf()` static method returns an array
containing those of the provided locales that are supported in
segmentation without having to fall back to the runtime\'s default
locale.
...
are supported in segmentation without having to fall back to the
...
in segmentation, `supportedLocalesOf` returns the Indonesian and German
language tags unchanged, even though `pinyin` collation is neither
relevant to segmentation nor used with Indonesian, and a specialized
...
--------------
  ...
  [\#
  sec-intl.segmenter.supportedlocalesof]](https://tc39.es/ecma402/#sec-intl.segmenter.supportedlocalesof)
