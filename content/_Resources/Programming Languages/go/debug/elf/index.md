...
DynString returns the strings listed for the given tag in the file\'s
dynamic section.
...
Values that are shared by both R\_PPC and R\_PPC64 are prefixed with
R\_POWERPC\_ in the ELF standard. For the R\_PPC type, the relevant
shared relocations have been renamed with the prefix R\_PPC\_. The
original name follows the value in a comment.
...
Values that are shared by both R\_PPC and R\_PPC64 are prefixed with
R\_POWERPC\_ in the ELF standard. For the R\_PPC64 type, the relevant
shared relocations have been renamed with the prefix R\_PPC64\_. The
original name follows the value in a comment.
...
### func (R\_PPC64) GoString 

```go
func (i R_PPC64) GoString() string
```

### func (R\_PPC64) String 
...
### func (R\_RISCV) GoString 

```go
func (i R_RISCV) GoString() string
```

### func (R\_RISCV) String 
...
### func (R\_SPARC) GoString 

```go
func (i R_SPARC) GoString() string
```

### func (R\_SPARC) String 
...
### func (R\_X86\_64) GoString 

```go
func (i R_X86_64) GoString() string
```

### func (R\_X86\_64) String 
...
Data reads and returns the contents of the ELF section. Even if the
section is stored compressed in the ELF file, Data returns uncompressed
data.

For an SHT\_NOBITS section, Data always returns a non-nil error.
...
Open returns a new ReadSeeker reading the ELF section. Even if the
section is stored compressed in the ELF file, the ReadSeeker reads
uncompressed data.

For an SHT\_NOBITS section, all calls to the opened reader will return a
non-nil error.
