...
Float64x2List.view constructor
==============================
...
Creates a [Float64x2List](../float64x2list-class) *view* of the
specified region in `buffer`.

Changes in the [Float64x2List](../float64x2list-class) will be visible
in the byte buffer and vice versa. If the `offsetInBytes` index of the
...
Merely doing `Float64x2List.view(other.buffer, 0, count)` may not point
to the bytes you intended. Instead you may need to do:
...
Alternatively, use
[Float64x2List.sublistView](float64x2list.sublistview) which includes
this computation:
...
<https://api.dart.dev/stable/2.18.5/dart-typed_data/Float64x2List/Float64x2List.view.html>
:::
