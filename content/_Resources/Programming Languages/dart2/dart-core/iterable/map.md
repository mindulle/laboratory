...
[Iterable](../iterable-class)\<T\> map\<T\>(
...
Returns a new lazy [Iterable](../iterable-class) with elements that are
created by calling `toElement` on each element of this `Iterable` in
iteration order.
...
Iterating multiple times over the returned [Iterable](../iterable-class)
will invoke the supplied `toElement` function once per element for on
each iteration.
...
<https://api.dart.dev/stable/2.18.5/dart-core/Iterable/map.html>
:::
