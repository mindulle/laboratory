...
The `grid-column` CSS [shorthand property](shorthand_properties.md)
specifies a grid item\'s size and location within a [grid
...
`grid-column-start` longhand is set to the value before the slash, and
the `grid-column-end` longhand is set to the value after the slash.
...
:   If there is a named line with the name
    ...
    **Note:** Named grid areas automatically generate implicit named
    lines of this form, so specifying `grid-column: foo;` will choose
    the start/end edge of that named grid area (unless another line
    named `foo-start`/`foo-end` was explicitly specified before it).
