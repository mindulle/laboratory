...
The `grid-row-start` CSS property specifies a grid item\'s start
position within the grid row by contributing a line, a span, or nothing
...
:   If there is a named line with the name \'\<custom-ident\>-start\',
    ...
    **Note:** Named grid areas automatically generate implicit named
    lines of this form, so specifying `grid-row-start: foo;` will choose
    the start edge of that named grid area (unless another line named
...
:   Contributes a grid span to the grid item\'s placement; such that the
    row start edge of the grid item\'s grid area is n lines from the end
    edge.
...
- Related CSS properties: [`grid-row-end`](grid-row-end.md),
